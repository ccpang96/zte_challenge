...
{
	map[f(*current)].max_t1_id1.insert((*current)[0]);
	map[f(*current)].min_t2_id1.insert((*current)[2]);
...
vector<vector<int>> mapToVector(map<int, map<int, group>> &result_map) {
	...
	for (auto it1 = result_map.begin(); it1 != result_map.end(); it1++) {
		for (auto it2 = it1->second.begin(); it2 != it1->second.end(); it2++) {
			cout << it1->first << ", " << it2->first << ", " << *((it2->second).max_t1_id1.begin()) << ", " << *((it2->second).min_t2_id1.begin()) << endl;
			result.emplace_back(vector<int>{it1->first, it2->first, *((it2->second).max_t1_id1.begin()), *((it2->second).min_t2_id1.begin())});
		}
	}
	return result;
}
...
void do_my_function(tab_t&& join_Result) {
	...
	vector<vector<int>> groupResult = mapToVector(result_map); //20ms
															   //vector<vector<int>> groupResult = groupBy2(join_Result);
															   //order by函数
